 ...
 * @brief   ILI9341 LCD 드라이버 구현 (SPI2 + DMA)
 * @details Landscape 320×240, RGB565, FreeRTOS 세마포어 기반 DMA 동기화.
...
void LCD_DrawChar(uint16_t x, uint16_t y, char ch, uint16_t fg, uint16_t bg)
{
    ...
    for (uint16_t row = 0; row < s_font->height; row++) {
        ...
        for (uint16_t col = 0; col < s_font->width; col++) {
            ...
            uint16_t color = (rowData[byteIdx] & (1 << bitIdx)) ? fg : bg;
            s_lineBuf[col * 2]     = (uint8_t)(color >> 8);
            s_lineBuf[col * 2 + 1] = (uint8_t)(color & 0xFF);
        }
